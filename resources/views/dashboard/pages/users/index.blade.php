...
@section('content')
    ...
    <div class="d-flex justify-content-end">
        @if (App\Helpers\Helpers::perUSer('users.create'))
            <x-create-button title="User" route="users.create" />
        @endif
        <button type="button" id="settingBtn" class="btn btn-primary btn-sm mx-2">Save Setting</button>
    </div>
    <!-- modal for export-->
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('users.export') }}">
                    @csrf
...
<script>
    ...
    $('#users_table').on('page.dt', function() {
        $('.selected-item').text(window.LaravelDataTables['users_table'].rows({
            selected: true
        }).count());
        $('.selected-badge').text(window.LaravelDataTables['users_table'].rows({
            selected: true
        }).count());
    });
   ...
   <label for=""${id}"">  ${$(this).text()}  <label>
            ...
            action: function(e, dt, node, config, cb) {
                ...
                $('#' + id).prop('checked', function(_, oldProp) {
                    if (oldProp) {
                        window.LaravelDataTables['users_table'].columns(index + 1).search(
                            "").draw();
    ...
    let exportFormat;
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('users_checkBoxIdsArray'));
