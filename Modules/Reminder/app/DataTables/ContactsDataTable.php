...
class ContactsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query->where('created_by', auth()->id())->orderBy('id', 'asc')))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('contacts.edit')) {
                    $html .= '<a href="' . route('contacts.edit', ['contact' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                /* if (Helpers::perUser('contacts.show')) {
                    $html .= '<a href="' . route('contacts.show', ['contact' => $model]) . '" class="dropdown-item">Show</a>';
                } */
                if (Helpers::perUser('contacts.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-contact" data-id="' . $model->id . '" data-url="' . route('contacts.destroy', ['contact' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
            ...
            ->addColumn('phone', function ($model) {
                ...
                if ($phones->count() > 0) {
                    $phoneNumbers = $phones->pluck('phone')->implode(', ');
                    return $phoneNumbers;
                } else {
                    return 'N/A'; // or any default value if there are no phones
                }
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['contacts-table'].columns.adjust().draw();
            window.LaravelDataTables['contacts-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['contacts-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['contacts-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['contacts-table'].button(2).enable(count > 0);
            window.LaravelDataTables['contacts-table'].button('export:name').enable(count > 0);

            checkBoxIds.clear();

            // Iterate over each selected row
            window.LaravelDataTables['contacts-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['contacts-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('contacts_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['contacts-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['contacts-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['contacts-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['contacts-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50, 'All']
    ]",
