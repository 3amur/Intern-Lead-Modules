...
class LeadAccountDataImport implements ToCollection, WithChunkReading, WithHeadingRow, SkipsEmptyRows
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $number => $row) {
            ...
            $array['created_as'] = 'lead';
            $array['condition'] = 'lead';
            DB::beginTransaction();
            try {
                if (!$validator->fails()) {
                    ...
                    $leadAccount = LeadAccount::where('account_name', $accountName)->where('created_by', auth()->id())->first();
                    if (!empty($leadAccount)) {
                        $array['is_new'] = false;
                        ...
                        //==============================
                        if (!empty($array['account_name']) && trim($array['account_name']) != trim($leadAccount->account_name)) {
                            $array['old_account_name'] = $leadAccount->account_name;
                            $array['has_changes'] = true;
                        }
                        if (!empty($array['account_contact_name']) && trim($array['account_contact_name']) != trim($leadAccount->account_contact_name)) {
                            $array['old_account_contact_name'] = $leadAccount->account_contact_name;
                            $array['has_changes'] = true;
                        }

                        if (!empty($array['personal_number']) && trim($array['personal_number']) != trim($leadAccount->personal_number)) {
                            $array['old_personal_number'] = $leadAccount->personal_number;
                            $array['has_changes'] = true;
                        }
                        if (!empty($array['condition']) && trim($array['condition']) != trim($leadAccount->condition)) {
                            $array['old_condition'] = $leadAccount->condition;
                            $array['has_changes'] = true;
                        }
                    ...
                    $founded_lead_accounts = LeadAccountImport::where('personal_number', $array['personal_number'])->where('created_as', 'lead')->first();

                    if (empty($founded_lead_accounts) && empty($founded_lead_accounts->id)) {
                        $leadAccount = LeadAccountImport::create($array);
                    }
                } else {
                    if (count($validator->errors()->all()) > 0) {
                        $fails_rows_array_with_errors[$number] = $validator->errors()->all();
                        $invalid_data_array = [];
                        $invalid_data_array['model'] = 'lead_Account';
