...
class LeadSourceDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query->where('created_by', auth()->id())))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('lead_source.edit')) {
                    $html .= '<a href="' . route('lead_source.edit', ['lead_source' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('lead_source.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-lead_source" data-id="' . $model->id . '" data-url="' . route('lead_source.destroy', ['lead_source' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['lead-source-table'].columns.adjust().draw();
            window.LaravelDataTables['lead-source-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['lead-source-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['lead-source-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['lead-source-table'].button(2).enable(count > 0);
            window.LaravelDataTables['lead-source-table'].button('export:name').enable(count > 0);

            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['lead-source-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['lead-source-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('lead_source_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['lead-source-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['lead-source-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['lead-source-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['lead-source-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50, 'All']
    ]",
