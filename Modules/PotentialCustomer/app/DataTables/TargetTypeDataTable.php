...
class TargetTypeDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($model) {
            ...
            if (Helpers::perUser('target_types.edit')) {
                $html .= '<a href="' . route('target_types.edit', ['target_type' => $model]) . '" class="dropdown-item">Edit</a>';
            }
            if (Helpers::perUser('target_types.destroy')) {
                $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-target_type" data-id="' . $model->id . '" data-url="' . route('target_types.destroy', ['target_type' => $model]) . '">Delete</a></div></div>';
            }
            return $html;
        })
        ...
        ->editColumn('status', function ($model) {
            ...
            ->editColumn('image', function ($model) {
                if(!empty($model->image))
                {
                    ...
                    return '<img width="50px" height:"50px" src="' . asset('dashboard/assets/img/blank-photo.jpg') . '" >';
                }
            })
     ...
     */
    public function query(TargetType $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['target_type-table'].columns.adjust().draw();
            window.LaravelDataTables['target_type-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['target_type-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['target_type-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['target_type-table'].button(2).enable(count > 0);
            window.LaravelDataTables['target_type-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['target_type-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['target_type-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('target_types_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['target_type-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['target_type-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['target_type-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['target_type-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200],
    ]",
    ...
    {
        return 'TaregtType_' . date('YmdHis');
    }
}
