...
class LeadTypeDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query->where('created_by', auth()->id())))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('lead_type.edit')) {
                    $html .= '<a href="' . route('lead_type.edit', ['lead_type' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('lead_type.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-lead_type" data-id="' . $model->id . '" data-url="' . route('lead_type.destroy', ['lead_type' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['lead-type-table'].columns.adjust().draw();
            window.LaravelDataTables['lead-type-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['lead-type-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['lead-type-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['lead-type-table'].button(2).enable(count > 0);
            window.LaravelDataTables['lead-type-table'].button('export:name').enable(count > 0);

            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['lead-type-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['lead-type-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('lead_type_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['lead-type-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['lead-type-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['lead-type-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['lead-type-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50, 'All']
    ]",
