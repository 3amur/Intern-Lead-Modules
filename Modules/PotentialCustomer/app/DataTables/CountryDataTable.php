...
class CountryDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('action', function ($model) {
                ...
                if (Helpers::perUser('countries.edit')) {
                    $html .= '<a href="' . route('countries.edit', ['country' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('countries.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-country" data-id="' . $model->id . '" data-url="' . route('countries.destroy', ['country' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['country-table'].columns.adjust().draw();
            window.LaravelDataTables['country-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['country-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['country-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['country-table'].button(2).enable(count > 0);
            window.LaravelDataTables['country-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();

            // Iterate over each selected row
            window.LaravelDataTables['country-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['country-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('countries_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['country-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['country-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['country-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['country-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50, 100,200]
    ]",
