...
class CityDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($model) {
            ...
            if (Helpers::perUser('cities.edit')) {
                $html .= '<a href="' . route('cities.edit', ['city' => $model]) . '" class="dropdown-item">Edit</a>';
            }
            if (Helpers::perUser('cities.destroy')) {
                $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-city" data-id="' . $model->id . '" data-url="' . route('cities.destroy', ['city' => $model]) . '">Delete</a></div></div>';
            }
            return $html;
        })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['city-table'].columns.adjust().draw();
            window.LaravelDataTables['city-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['city-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['city-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['city-table'].button(2).enable(count > 0);
            window.LaravelDataTables['city-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();

            // Iterate over each selected row
            window.LaravelDataTables['city-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['city-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('cities_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['city-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['city-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['city-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['city-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200]
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('state')->addClass('text-center'),
