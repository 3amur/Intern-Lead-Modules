...
class LeadValueDataTable extends DataTable
{
    /**
    ...
    {
        return (new EloquentDataTable($query->where('created_by',auth()->id())))
        ->addColumn('action', function ($model) {
            ...
            if (Helpers::perUser('lead_value.edit')) {
                $html .= '<a href="' . route('lead_value.edit', ['lead_value' => $model]) . '" class="dropdown-item">Edit</a>';
            }
            if (Helpers::perUser('lead_value.destroy')) {
                $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-lead_value" data-id="' . $model->id . '" data-url="' . route('lead_value.destroy', ['lead_value' => $model]) . '">Delete</a></div></div>';
            }
            return $html;
        })
     ...
     */
    public function query(LeadValue $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['lead-value-table'].columns.adjust().draw();
            window.LaravelDataTables['lead-value-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['lead-value-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['lead-value-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['lead-value-table'].button(2).enable(count > 0);
            window.LaravelDataTables['lead-value-table'].button('export:name').enable(count > 0);

            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['lead-value-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['lead-value-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('lead_value_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['lead-value-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['lead-value-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['lead-value-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['lead-value-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50, 'All']
    ]",
