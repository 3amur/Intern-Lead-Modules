...
class BrokerTypesDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->addColumn('action', function ($model) {
            ...
            if (Helpers::perUser('broker_types.edit')) {
                $html .= '<a href="' . route('broker_types.edit', ['broker_type' => $model]) . '" class="dropdown-item">Edit</a>';
            }
            if (Helpers::perUser('broker_types.destroy')) {
                $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-broker_type" data-id="' . $model->id . '" data-url="' . route('broker_types.destroy', ['broker_type' => $model]) . '">Delete</a></div></div>';
            }
            return $html;
        })
     ...
     */
    public function query(BrokerType $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['broker_types-table'].columns.adjust().draw();
            window.LaravelDataTables['broker_types-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['broker_types-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['broker_types-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['broker_types-table'].button(2).enable(count > 0);
            window.LaravelDataTables['broker_types-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['broker_types-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['broker_types-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('broker_types_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['broker_types-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['broker_types-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['broker_types-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['broker_types-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200],
    ]",
    ...
    {
        return 'BrokerTypes_' . date('YmdHis');
    }
}
