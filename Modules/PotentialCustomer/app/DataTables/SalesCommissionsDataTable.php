...
class SalesCommissionsDataTable extends DataTable
{
    /**
    ...
    {
        return(new EloquentDataTable($query->where('sales_agent_id',request('sales_agent')->id)))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('sales_commissions.edit')) {
                    $html .= '<a href="' . route('sales_commissions.edit', ['sales_commission' => $model->id,'sales_agent'=>request('sales_agent')->id]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('sales_commissions.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-sales_commissions" data-id="' . $model->id . '" data-url="' . route('sales_commissions.destroy', ['sales_commission' => $model->id,'sales_agent'=>request('sales_agent')->id]) . '">Delete</a></div></div>';
                }
               // return $html;
            })
     ...
     */
    public function query(SalesCommission $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['sales_commission-table'].columns.adjust().draw();
            window.LaravelDataTables['sales_commission-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['sales_commission-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['sales_commission-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['sales_commission-table'].button(2).enable(count > 0);
            window.LaravelDataTables['sales_commission-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['sales_commission-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['sales_commission-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('sales_commissions_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['sales_commission-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['sales_commission-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
        ...
        return $this->builder()
            ->setTableId('sales_commission-table')
      ...
      $('.selected-item').text(window.LaravelDataTables['sales_commission-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['sales_commission-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200],
    ]",
    ...
    {
        return 'SalesCommissions_' . date('YmdHis');
    }
}
