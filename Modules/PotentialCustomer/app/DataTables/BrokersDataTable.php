...
class BrokersDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('brokers.show')) {
                    $html .= '<a href="' . route('brokers.show', ['broker' => $model->id]) . '" class="dropdown-item">Show Details</a>';
                }
                if (Helpers::perUser('broker_commissions.index')) {
                    $html .= '<a href="' . route('broker_commissions.index', ['broker' => $model->id]) . '" class="dropdown-item">Broker Commissions</a>';
                }
                if (Helpers::perUser('brokers.edit')) {
                    $html .= '<a href="' . route('brokers.edit', ['broker' => $model->id]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('brokers.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-broker" data-id="' . $model->id . '" data-url="' . route('brokers.destroy', ['broker' => $model->id]) . '">Delete</a></div></div>';
                }
                return $html;
            })
            ...
            ->addColumn('broker_type',function($model){
                return $model->brokerType ? $model->brokerType->title : 'N/A';
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['brokers-table'].columns.adjust().draw();
            window.LaravelDataTables['brokers-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['brokers-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['brokers-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['brokers-table'].button(2).enable(count > 0);
            window.LaravelDataTables['brokers-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['brokers-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['brokers-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(1) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('brokers_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['brokers-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['brokers-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['brokers-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['brokers-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200],
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('broker_type')->addClass('text-center'),
