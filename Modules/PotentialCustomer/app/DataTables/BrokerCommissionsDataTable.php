...
class BrokerCommissionsDataTable extends DataTable
{
    /**
    ...
    {
        return(new EloquentDataTable($query->where('broker_id',request('broker')->id)))
        ...
        <div class="dropdown-menu dropdown-menu-end py-2">';
            if (Helpers::perUser('broker_commissions.edit')) {
                $html .= '<a href="' . route('broker_commissions.edit', ['broker_commission' => $model->id,'broker'=>request('broker')]) . '" class="dropdown-item">Edit</a>';
            }
            if (Helpers::perUser('broker_commissions.destroy')) {
                $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-broker_commissions" data-id="' . $model->id . '" data-url="' . route('broker_commissions.destroy', ['broker_commission' => $model->id,'broker'=>request('broker')]) . '">Delete</a></div></div>';
            }
            //return $html;
        })
        ...
        ->addColumn('broker_agent', function ($model) {
            return $model->broker->name;
        })
     ...
     */
    public function query(BrokerCommission $model): QueryBuilder
    {
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['broker_commissions-table'].columns.adjust().draw();
            window.LaravelDataTables['broker_commissions-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['broker_commissions-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['broker_commissions-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['broker_commissions-table'].button(2).enable(count > 0);
            window.LaravelDataTables['broker_commissions-table'].button('export:name').enable(count > 0);

            checkBoxIds.clear();

            // Iterate over each selected row
            window.LaravelDataTables['broker_commissions-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['broker_commissions-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('broker_commissions_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['broker_commissions-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['broker_commissions-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
        ...
        return $this->builder()
            ->setTableId('broker_commissions-table')
      ...
      $('.selected-item').text(window.LaravelDataTables['broker_commissions-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['broker_commissions-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200]
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('broker_agent')->addClass('text-center'),
    ...
    {
        return 'BrokerCommissions_' . date('YmdHis');
    }
}
