...
class SalesTargetDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query->where('created_by', auth()->id())->orderBy('id', 'asc')))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('sales_targets.edit')) {
                    $html .= '<a href="' . route('sales_targets.edit', ['sales_target' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('sales_targets.show')) {
                    $html .= '<a href="' . route('sales_targets.show', ['sales_target' => $model]) . '" class="dropdown-item">Show Details</a>';
                }
                if (Helpers::perUser('sales_targets.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-sales_target" data-id="' . $model->id . '" data-url="' . route('sales_targets.destroy', ['sales_target' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
            ...
            ->editColumn('target_name', function ($model) {
                return $model->target_name;
            })
            ->editColumn('target_value', function ($model) {
                return $model->target_value;
            })
            ->editColumn('target_duration', function ($model) {
                return $model->start_date . ' To ' . $model->end_date;
            })

            ->addColumn('sales_agents', function ($model) {
                $salesAgents = $model->salesAgents;
                $salesAgentNames = $salesAgents->pluck('name')->implode(', ');

                return $salesAgentNames;
            })
            ->addColumn('target_types', function ($model) {
                $targetTypes = $model->targetTypes;
                $targetTypeTitles = $targetTypes->pluck('title')->implode(', ');

                return $targetTypeTitles;
            })


            ->editColumn('target_calculation_method', function ($model) {
                return $model->target_calc_method;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            window.LaravelDataTables['sales_target-table'].columns.adjust().draw();
            window.LaravelDataTables['sales_target-table'].on('select deselect',function(){
            var count = window.LaravelDataTables['sales_target-table'].rows({ selected: true }).count();
            $('.selected-item').text(count);
            $('.selected-badge').text(count);
            window.LaravelDataTables['sales_target-table'].button('copy:name').enable(count > 0);
            window.LaravelDataTables['sales_target-table'].button(2).enable(count > 0);
            window.LaravelDataTables['sales_target-table'].button('export:name').enable(count > 0);
            checkBoxIds.clear();
            // Iterate over each selected row
            window.LaravelDataTables['sales_target-table'].rows({ selected: true }).every(function(rowIdx) {
            ...
            window.LaravelDataTables['sales_target-table'].rows({ selected: false }).every(function(rowIdx) {
                // Get the corresponding checkbox and check it
                var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('sales_targets_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                })
                        ...
                        arrOfFilterBtn.push({
                            ...
                            action: function (e, dt, node, config, cb) {
                                ...
                                $('#' + id).prop('checked', function (_, oldProp) {
                                    if (oldProp) {
            window.LaravelDataTables['sales_target-table'].columns(index + 1).search("").draw();
                ...
                arrOfFilterBtn.push({
                    ...
                    action: function(e, dt, node, config, cb) {
                        ...
                        $('.filter-modal input[type="text"]').on('input', function() {
                            ...
                            window.LaravelDataTables['sales_target-table'].columns(filterIndex).search(searchValue).draw();
                        });
                    }
                });
      ...
      $('.selected-item').text(window.LaravelDataTables['sales_target-table'].rows({ selected: true }).count());
      $('.selected-badge').text(window.LaravelDataTables['sales_target-table'].rows({ selected: true }).count());
    },lengthMenu: [
        [10, 25, 50,100,200],
        [10, 25, 50,100,200],
    ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('target_name')->addClass('text-center'),
            Column::make('target_value')->addClass('text-center'),
            Column::make('target_duration')->addClass('text-center'),
            Column::make('sales_agents')->addClass('text-center'),
            Column::make('target_types')->addClass('text-center'),
            Column::make('target_calculation_method')->addClass('text-center'),
