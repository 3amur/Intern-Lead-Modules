...
class StateDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            <div class="dropdown-menu dropdown-menu-end py-2">';
                if (Helpers::perUser('states.edit')) {
                    $html .= '<a href="' . route('states.edit', ['state' => $model]) . '" class="dropdown-item">Edit</a>';
                }
                if (Helpers::perUser('states.destroy')) {
                    $html .= '<div class="dropdown-divider"></div><a href="#" class="dropdown-item text-danger delete-this-state" data-id="' . $model->id . '" data-url="' . route('states.destroy', ['state' => $model]) . '">Delete</a></div></div>';
                }
                return $html;
            })
    ...
    public function html(): HtmlBuilder
    {
        ...
        $initCompleteFunction = "function() {
            ...
            var checkBoxIds = new Set();
             ...
             window.LaravelDataTables['state-table'].columns.adjust().draw();
             window.LaravelDataTables['state-table'].on('select deselect',function(){
             var count = window.LaravelDataTables['state-table'].rows({ selected: true }).count();
             $('.selected-item').text(count);
             $('.selected-badge').text(count);
             window.LaravelDataTables['state-table'].button('copy:name').enable(count > 0);
             window.LaravelDataTables['state-table'].button(2).enable(count > 0);
             window.LaravelDataTables['state-table'].button('export:name').enable(count > 0);


             var checkBoxIds = new Set();             // Iterate over each selected row
             window.LaravelDataTables['state-table'].rows({ selected: true }).every(function(rowIdx) {
             ...
             window.LaravelDataTables['state-table'].rows({ selected: false }).every(function(rowIdx) {
                 // Get the corresponding checkbox and check it
                 var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
             ...
             localStorage.setItem('states_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                 })
                         ...
                         arrOfFilterBtn.push({
                             ...
                             action: function (e, dt, node, config, cb) {
                                 ...
                                 $('#' + id).prop('checked', function (_, oldProp) {
                                     if (oldProp) {
             window.LaravelDataTables['state-table'].columns(index + 1).search("").draw();
                 ...
                 arrOfFilterBtn.push({
                     ...
                     action: function(e, dt, node, config, cb) {
                         ...
                         $('.filter-modal input[type="text"]').on('input', function() {
                             ...
                             window.LaravelDataTables['state-table'].columns(filterIndex).search(searchValue).draw();
                         });
                     }
                 });
       ...
       $('.selected-item').text(window.LaravelDataTables['state-table'].rows({ selected: true }).count());
       $('.selected-badge').text(window.LaravelDataTables['state-table'].rows({ selected: true }).count());
     },lengthMenu: [
         [10, 25, 50,100,200],
         [10, 25, 50,100,200]
     ]",
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('country')->addClass('text-center'),
