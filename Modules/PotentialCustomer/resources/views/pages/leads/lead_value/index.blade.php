...
@section('content')
    ...
    <div class="d-flex justify-content-end">
        ...
        @if (app\Helpers\Helpers::perUser('lead_value.create'))
            <x-potentialcustomer::create-button route="{{ route('lead_value.create') }}" title="Lead Values" />
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('lead_value.export') }}">
                    @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-this-lead_value', function(e) {
                e.preventDefault();
                let el = $(this);
                let url = el.attr('data-url');
                let id = el.attr('data-id');
                swalWithBootstrapButtons.fire({
                    title: "Are you sure you really want to delete this Lead Value?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(msg) {
                                window.location.href = "{{ route('lead_value.index') }}";
                            }
                        });
                    } else if (
    ...
    <script>
        ...
        $('#lead-value-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['lead-value-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['lead-value-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['lead-value-table'].columns(index + 2).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('lead_value_checkBoxIdsArray'));
