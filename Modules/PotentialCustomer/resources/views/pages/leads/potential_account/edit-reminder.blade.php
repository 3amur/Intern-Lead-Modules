...
@section('content')
    ...
    <x-potentialcustomer::breadcrumb>
        ...
        <x-potentialcustomer::breadcrumb-item>
            <a
                href="{{ route('potential_account.show', ['potential_account' => $potentialAccount]) }}">{{ __($potentialAccount->account_name) }}</a>
        </x-potentialcustomer::breadcrumb-item>
    ...
    <div class="container">
        ...
        <div class="card my-3">
            ...
            <form
                action="{{ route('potential_accounts.updateCustomerReminder', ['reminder' => $reminder, 'potential_account' => $potentialAccount->id]) }}"
                method="POST" enctype="multipart/form-data" id="reminderForm">
                @csrf
                @method('PUT')
                <div class="card-body">
                    <x-potentialcustomer::form-input type="text" :value="isset($reminder) ? Str::ucfirst($reminder->reminder_type) : old('reminder_type')" label="Reminder Type"
                        name='reminder_type' placeholder='Reminder Type' required id="reminder_type"
                        oninput="{{ null }}" readonly />
                    <input type="text" name="reminder_relation" value="leads" hidden>
                    <input type="text" name="lead_id" value="{{ $potentialAccount->id }}" hidden>
                    <hr>
