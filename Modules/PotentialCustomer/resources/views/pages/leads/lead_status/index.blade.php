...
@section('content')
    ...
    <h4>{{ __('Lead Status') }}</h4>
    <div class="d-flex justify-content-end">

        @if (app\Helpers\Helpers::perUser('lead_status.create'))
            <x-potentialcustomer::create-button route="{{ route('lead_status.create') }}" title="Lead Statuss" />
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('lead_status.export') }}">
                    @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-this-lead_status', function(e) {
                e.preventDefault();
                let el = $(this);
                ...
                swalWithBootstrapButtons.fire({
                    title: "Are you sure you really want to delete this Lead Status?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(msg) {
                                window.location.href = "{{ route('lead_status.index') }}";
                            }
                        });
                    } else if (
    ...
    <script>
        ...
        $('#lead-status-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['lead-status-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['lead-status-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['lead-status-table'].columns(index + 2).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('lead_status_checkBoxIdsArray'));
