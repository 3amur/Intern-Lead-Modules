...
@section('content')
    ...
    <div class="d-flex justify-content-end">
        ...
        @if (app\Helpers\Helpers::perUser('lead_source.create'))
            <x-potentialcustomer::create-button route="{{ route('lead_source.create') }}" title="Lead Sources" />
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('lead_source.export') }}">
                    @csrf
...
@section('js')
    ...
    <script>
        ...
        $('#lead-source-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['lead-source-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['lead-source-table'].rows({
                selected: true
            }).count());
        });
            ...
            <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['lead-source-table'].columns(index + 1)
                                .search(
                                    "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('lead_source_checkBoxIdsArray'));
