...
@section('content')
    ...
    {{-- End breadcrumb --}}
            <h4>{{ __('Sales Commissions') }}</h4>
            <div class="d-flex justify-content-end">

        @if(app\Helpers\Helpers::perUser('sales_commissions.create'))
        <x-potentialcustomer::create-button route="{{ route('sales_commissions.create',['sales_agent'=>$salesAgent->id]) }}" title="Sales Commission"/>
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('sales_commissions.export',['sales_agent'=>$salesAgent->id]) }}">
                    @csrf
...
{{--     <script>
    ...
    <script>
        ...
        $('#sales_commission-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['sales_commission-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['sales_commission-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['sales_commission-table'].columns(index + 1).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('sales_commissions_checkBoxIdsArray'));
