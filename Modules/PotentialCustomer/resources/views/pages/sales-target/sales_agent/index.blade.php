...
@section('content')
    ...
    {{-- End breadcrumb --}}
            <h4>{{ __('Sales Agents') }}</h4>
            <div class="d-flex justify-content-end">
       {{-- @if(app\Helpers\Helpers::perUser('sales_agents.create'))
                    <x-potentialcustomer::create-button route="sales_agents" title="Sales Agents"/>
                    @endif --}}
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('sales_agents.export') }}">
                    @csrf
...
@section('js')
    ...
    <script>
        ...
        $('#sales_agents-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['sales_agents-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['sales_agents-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['sales_agents-table'].columns(index + 1).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('sales_agents_checkBoxIdsArray'));
