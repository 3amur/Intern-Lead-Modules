...
@section('content')
    ...
    <x-potentialcustomer::breadcrumb>
        ...
        <x-potentialcustomer::breadcrumb-item>
            {{ __('Broker Commissions') }}
        </x-potentialcustomer::breadcrumb-item>
    </x-potentialcustomer::breadcrumb>
    {{-- End breadcrumb --}}

            <h4>{{ __('Broker Commissions') }}</h4>
            <div class="d-flex justify-content-end">
        @if(app\Helpers\Helpers::perUser('broker_commissions.create'))
        <x-potentialcustomer::create-button route="{{ route('broker_commissions.create',['broker'=>$broker->id]) }}" title="Broker Commission"/>
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('broker_commissions.export',['broker'=>$broker->id]) }}">
                    @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-this-lead_source', function(e) {
                e.preventDefault();
                let el = $(this);
                let url = el.attr('data-url');
                let id = el.attr('data-id');
                swalWithBootstrapButtons.fire({
                    title: "Are you sure you really want to delete this Lead Source?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(msg) {
                                window.location.href = "{{ route('lead_source.index') }}";
                            }
                        });
                    } else if (
    ...
    <script>
        ...
        $('#broker_commissions-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['broker_commissions-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['broker_commissions-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['broker_commissions-table'].columns(index + 1).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('broker_commissions_checkBoxIdsArray'));
