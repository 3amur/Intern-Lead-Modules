...
@section('title')
    {{ isset($brokerCommission) ? __('Edit Broker Commission ') : __('Create New Broker Commission') }}
@endsection
@section('css')
...
@section('content')
    ...
    <x-potentialcustomer::breadcrumb>
        ...
        <x-potentialcustomer::breadcrumb-item>
            <a href="{{ route('broker_commissions.index', ['broker' => $broker->id]) }}">{{ __('Broker Commission') }}</a>
        </x-potentialcustomer::breadcrumb-item>

        <x-potentialcustomer::breadcrumb-item>
            {{ isset($brokerCommission) ? __('Edit :type', ['type' => $brokerCommission->title]) : __('Create New Broker Commission') }}
        </x-potentialcustomer::breadcrumb-item>
    </x-potentialcustomer::breadcrumb>
    {{-- End breadcrumb --}}
    <div class="container">
        ...
        <div class="card my-3">
            ...
            <form
                action="{{ isset($brokerCommission) ? route('broker_commissions.update', ['broker_commission' => $brokerCommission, 'broker' => $broker->id]) : route('broker_commissions.store', ['broker' => $broker->id]) }}"
                method="POST" enctype="multipart/form-data" id="brokerCommissionForm">
                @csrf
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-6">
                            <x-potentialcustomer::form-input type="text" :value="isset($brokerCommission) ? $brokerCommission->title : old('title')" label="Commission Title"
                                name='title' placeholder='Broker Commission Title' id="commission_title"
                                oninput="{{ null }}" required />
                        </div>
                        <div class="col-6">
                            <x-potentialcustomer::form-select name='status' id="status" label="status" required>
                                <option @if (isset($brokerCommission) && $brokerCommission->status == 'active') selected @endif value="active">
                                    {{ __('Active') }}</option>
                                <option @if (isset($brokerCommission) && $potentialAccount->status == 'inactive') selected @endif value="inactive">
                                    {{ __('Inactive') }}</option>
                                <option @if (isset($brokerCommission) && $brokerCommission->status == 'draft') selected @endif value="draft">
                                    {{ __('Draft') }}</option>
                    ...
                    <x-potentialcustomer::form-description
                        value="{{ isset($brokerCommission) ? $brokerCommission->notes : old('notes') }}" label="Notes"
                        name='notes' placeholder='Broker Commission notes' id="notes" />
                    ...
                    <div class="row my-4">
                        ...
                        <div class="col-6">
                            ...
                            <table>
                                <thead class="fs--1 thead" style="display: none">
                                    <tr>
                                ...
                                <tbody id="rowParent">
                                    @if (!empty($brokerCommission) && !empty($brokerCommission->commissionLayers))
                                        @foreach ($brokerCommission->commissionLayers as $index => $broker_commission_up_to)
                                            <tr>
                                                <td><input readonly type="text" data-amount_from="{{ $index + 1 }}"
                                                        ...
                                                        value="{{ number_format($broker_commission_up_to->amount_from, 2, '.', ',') }}"
                                                        required=""></td>
                                                <td><input readonly type="text" data-amount_to="{{ $index + 1 }}"
                                                        ...
                                                        @if ($broker_commission_up_to->amount_to == '1000000000000000.00') style="display:none" @endif
                                                        @if (!empty($broker_commission_up_to->amount_to) && $broker_commission_up_to->amount_to != '1000000000000000.00') value="{{ $broker_commission_up_to->amount_to ? number_format($broker_commission_up_to->amount_to, 2, '.', ',') : '' }}" @endif
                                                        required=""></td>
                                                <td><input readonly type="number" class="form-control"
                                                        ...
                                                        value="{{ $broker_commission_up_to->percent }}" required="">
                                                </td>
...
@section('js')
    ...
    @if (!empty($broker_commission_up_to->amount_to) && $broker_commission_up_to->amount_to == '1000000000000000.00')
        <script>
            unlimeted = 1;
        </script>
    @else
    ...
    <script>
        ...
        @if (!empty($brokerCommission) && !empty($brokerCommission->commissionLayers))
            var indexs = [];
            for (var i = 1; i <= '{{ $brokerCommission->commissionLayers->count() }}'; i++) {
                indexs.push(i);
            }
