...
@section('content')
    ...
    {{-- End breadcrumb --}}
            ...
            <h4>{{ __('Broker Type') }}</h4>
            <div class="d-flex justify-content-end">

        @if(app\Helpers\Helpers::perUser('broker_types.create'))
        <x-potentialcustomer::create-button route="{{ route('broker_types.create') }}" title="Broker Types"/>
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('broker_types.export') }}">
                    @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-this-broker_type', function(e) {
                e.preventDefault();
                let el = $(this);
                let url = el.attr('data-url');
                let id = el.attr('data-id');
                swalWithBootstrapButtons.fire({
                    title: "Are you sure you really want to delete this Broker Type?",
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(msg) {
                                window.location.href = "{{ route('broker_types.index') }}";
                            }
                        });
                    } else if (
    ...
    <script>
        ...
        $('#broker_types-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['broker_types-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['broker_types-table'].rows({
                selected: true
            }).count());
        });
       ...
       <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['broker_types-table'].columns(index + 1).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('broker_types_checkBoxIdsArray'));
