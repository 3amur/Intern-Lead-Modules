...
@section('content')
    ...
    <div class="d-flex justify-content-end">
        ...
        @if (app\Helpers\Helpers::perUser('countries.create'))
            <x-potentialcustomer::create-button title="country" route="{{ route('countries.create') }}" />
        @endif
    </div>
    <div>
    ...
    <div class="modal fade " id="exportModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="exportModalForm"  method="POST" action="{{ route('countries.export') }}">
                    @csrf
...
@section('js')
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.delete-this-country', function(e) {
                e.preventDefault();
                let el = $(this);
                let url = el.attr('data-url');
                let id = el.attr('data-id');
                swalWithBootstrapButtons.fire({
                    title: "Are you sure you really want to delete this Country?",
    ...
    <script>
        ...
        $('#country-table').on('page.dt', function() {
            $('.selected-item').text(window.LaravelDataTables['country-table'].rows({
                selected: true
            }).count());
            $('.selected-badge').text(window.LaravelDataTables['country-table'].rows({
                selected: true
            }).count());
        });
   ...
   <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        if (oldProp) {
                            window.LaravelDataTables['country-table'].columns(index + 1).search(
                                "").draw();
        ...
        $(document).on('click','.exportSelected',function() {
            let SelectedRows = JSON.parse(localStorage.getItem('countries_checkBoxIdsArray'));
