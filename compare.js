...
table.on('select deselect', function(e, dt, type, indexes) {
  ...
  table.rows({ selected: false }).every(function(rowIdx) {
    ...
    $(function(){window.LaravelDataTables=window.LaravelDataTables||{};window.LaravelDataTables["roles_table"]=$("#roles_table").DataTable({"serverSide":true,"processing":true,"ajax":{"url":"http:\/\/127.0.0.1:8000\/roles","type":"GET","data":function(data) {
        ...
        delete data.search.regex;}},"columns":[{"data":"","name":"","title":"","orderable":false,"searchable":false,"className":"text-center dtr-control"},{"data":"","name":"","title":"","orderable":false,"searchable":false,"className":"text-center"},{"data":"id","name":"id","title":"Id","orderable":true,"searchable":true,"className":"text-center"},{"data":"name","name":"name","title":"Name","orderable":true,"searchable":true,"className":"text-center"},{"data":"status","name":"status","title":"Status","orderable":true,"searchable":true,"className":"text-center"},{"data":"created_at","name":"created_at","title":"Created At","orderable":true,"searchable":true,"className":"text-center"},{"data":"updated_at","name":"updated_at","title":"Updated At","orderable":true,"searchable":true,"className":"text-center"},{"data":"action","name":"action","title":"Action","orderable":false,"searchable":false,"width":60,"className":"text-center"}],"order":[[3,"desc"]],"select":{"style":"multi","selector":"input[type=\"checkbox\"]"},"initComplete":function() {
        table.columns.adjust().draw();
        ...
        createExportModalElements();
    },"responsive":{"orderable":false,"details":{"target":0}},"layout":{"top2Start":{"buttons":[{"text":"Select All","action":function() {
                                            let allSelected = true;
                                            table.column(1).nodes().to$().find('input[type="checkbox"]').each(
                                                function() {
                                                    if (!$(this).prop('checked')) {
                                                        allSelected = false;
                                                        return false; // break out of the each loop
                                            ...
                                            if (allSelected) {
                                                table.rows().deselect();
                                                table.column(1).nodes().to$().find('input[type="checkbox"]').prop(
                                                    'checked', false);
                                            } else {
                                                table.rows().select();
                                                table.column(1).nodes().to$().find('input[type="checkbox"]').prop(
                                        ...
                                        },"className":"ms-1  rounded-1 mt-2"},{"extend":"copyHtml5","name":"copy","enabled":false,"text":function() {
                                            return '<span>Copy <span style="background :var(--phoenix-body-bg); color: var(--phoenix-1100) !important;  " class="badge badge-primary  ms-3 selected-badge">' +
                                                0 + '</span></span>';
                                        },"exportOptions":{"columns":":nth-child(n+3):not(:last-child):visible","modifier":{"selected":true}},"className":"ms-1  rounded-1 mt-2 "},{"extend":"print","name":"print","enabled":false,"text":function() {
                                            return '<span>Print <span style="background :var(--phoenix-body-bg); color: var(--phoenix-1100) !important;"  class="badge badge-primary text-black ms-3 selected-badge">' +
                                                0 + '</span></span>';
                                        },"exportOptions":{"columns":":nth-child(n+3):not(:last-child):visible","modifier":{"selected":true}},"className":"ms-1  rounded-1 mt-2"},{"text":"<i class=\"fa fa-refresh \" aria-hidden=\"true\"><\/i>","action":function(e, dt, node, config) {
                                            table.rows().deselect();
                                            table.column(1).nodes().to$().find('input[type="checkbox"]').prop('checked',
                                                false);

                                            dt.ajax.reload();
                                            table.draw();
                                            window.location.reload();
                                        },"className":"ms-1  rounded-1 mt-2"},{"extend":"colvis","popoverTitle":"<h5> Column visibility <\/h5>","className":"ms-1  rounded-1 mt-2"}]},"top2End":{"buttons":[{"extend":"collection","text":"Export","name":"export","popoverTitle":"<h5> Export <\/h5>","className":"ms-1  rounded-1 mt-2","enabled":false,"buttons":[{"text":function() {
                                                    return '<div data-bs-toggle="modal" data-bs-target="#exportModal"> Excel</div>'
                                                }},{"text":function() {
                                                    return '<div data-bs-toggle="modal" data-bs-target="#exportModal">PDF</div>'
                                                }},{"text":function() {
                                                    return '<div data-bs-toggle="modal" data-bs-target="#exportModal"> CSV</div>'
                                                }}]},{"extend":"collection","text":" Filter","popoverTitle":"<h5> Column Filter <\/h5>","className":"ms-1  rounded-1 mt-2  d-block ","buttons":"arrOfFilterBtn"}]}},"language":{"lengthMenu":"Show _MENU_ ","searchPlaceholder":"Users Search"},"columnDefs":[{"orderable":false,"render":"DataTable.render.select()","targets":1,"className":"select-checkbox"},{"orderable":false,"targets":0}],"stateSave":true,"stateSaveParams":function(settings, data) {
                                            data.columns.map(item => {
                                                item.search = '';
                                            });
                                            data.search.search = '';
   ...
   <label for=""${id}"">  ${$(this).text()}  <label>
                ...
                action: function(e, dt, node, config, cb) {
                    ...
                    $('#' + id).prop('checked', function(_, oldProp) {
                        ...
                        if (oldProp) {
                            table.columns(index + 2).search("").draw();
                            searchValues = searchValues.filter(item => item
                                .Column_No !==
                                index + 2);
                        }
                        console.log(index + 2);
            ...
            action: function(e, dt, node, config, cb) {
                ...
                checkedValues.forEach((element, i) => {
                    if (element.checked) {
                        ...
                        searchInput.setAttribute('id', columnTitleArr[i].toString().trim());
                        searchInput.setAttribute('type', 'text');
                        searchInput.setAttribute('placeholder', columnTitleArr[i] +
                            ' filter ');
                        oldValueFlag = searchValues.find(item => item.Column_No == i + 2)
                        if (oldValueFlag) {
                            searchInput.value = oldValueFlag.Search_Criteria
                        ...
                        console.log("the index of true element = " + columnTitleArr[i]
                            .toString()
                            .trim());
                    } else {
                ...
                $('.filter-modal input[type="text"]').on('input', function() {
                    ...
                    console.log(searchValue);
                    // Check if the filter already exists in searchValues
                    const existingFilterIndex = searchValues.findIndex(item => item
                        .Column_No ===
                        filterIndex);
                    if (existingFilterIndex !== -1) {
                        // If the filter exists, update its search criteria
                        ...
                        // If the filter doesn't exist, add it to searchValues
                        searchValues.push({
                            Column_No: filterIndex,
                            Search_Criteria: searchValue
                        });
                    }
        ...
        var table = window.LaravelDataTables["roles_table"];
        console.log(table);
        var state = table.state();
