     ...
     public function html(): HtmlBuilder
     {
         ...
         $initCompleteFunction = "function() {
            var checkBoxIds = new Set();
             ...
             window.LaravelDataTables['roles-table'].columns.adjust().draw();
             window.LaravelDataTables['roles-table'].on('select deselect',function(){
             var count = window.LaravelDataTables['roles-table'].rows({ selected: true }).count();
             $('.selected-item').text(count);
             $('.selected-badge').text(count);
             window.LaravelDataTables['roles-table'].button('copy:name').enable(count > 0);
             window.LaravelDataTables['roles-table'].button(2).enable(count > 0);
             window.LaravelDataTables['roles-table'].button('export:name').enable(count > 0);

             checkBoxIds.clear();
             // Iterate over each selected row
             window.LaravelDataTables['roles-table'].rows({ selected: true }).every(function(rowIdx) {
             ...
             window.LaravelDataTables['roles-table'].rows({ selected: false }).every(function(rowIdx) {
                 // Get the corresponding checkbox and check it
                 var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
            ...
            localStorage.setItem('roles_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                 })
                         ...
                         arrOfFilterBtn.push({
                             ...
                             action: function (e, dt, node, config, cb) {
                                 ...
                                 $('#' + id).prop('checked', function (_, oldProp) {
                                     if (oldProp) {
             window.LaravelDataTables['roles-table'].columns(index + 1).search("").draw();
                 ...
                 arrOfFilterBtn.push({
                     ...
                     action: function(e, dt, node, config, cb) {
                         ...
                         $('.filter-modal input[type="text"]').on('input', function() {
                             ...
                             window.LaravelDataTables['roles-table'].columns(filterIndex).search(searchValue).draw();
                         });
                     }
                 });
       ...
       $('.selected-item').text(window.LaravelDataTables['roles-table'].rows({ selected: true }).count());
       $('.selected-badge').text(window.LaravelDataTables['roles-table'].rows({ selected: true }).count());
     },lengthMenu: [
         [10, 25, 50,100,200],
         [10, 25, 50,100,200]
     ]",
