     ...
     public function html(): HtmlBuilder
     {
         ...
         $initCompleteFunction = "function() {
            ...
            var checkBoxIds = new Set();
             ...
             window.LaravelDataTables['users-table'].columns.adjust().draw();
             window.LaravelDataTables['users-table'].on('select deselect',function(){
             var count = window.LaravelDataTables['users-table'].rows({ selected: true }).count();
             $('.selected-item').text(count);
             $('.selected-badge').text(count);
             window.LaravelDataTables['users-table'].button('copy:name').enable(count > 0);
             window.LaravelDataTables['users-table'].button(2).enable(count > 0);
             window.LaravelDataTables['users-table'].button('export:name').enable(count > 0);

             checkBoxIds.clear();
             // Iterate over each selected row
             window.LaravelDataTables['users-table'].rows({ selected: true }).every(function(rowIdx) {
             ...
             window.LaravelDataTables['users-table'].rows({ selected: false }).every(function(rowIdx) {
                 // Get the corresponding checkbox and check it
                 var checkbox = $(this.node()).find('td:eq(0) input[type=\"checkbox\"]');
             ...
             localStorage.setItem('users_checkBoxIdsArray', JSON.stringify(checkBoxIdsArray))
                 })
                         ...
                         arrOfFilterBtn.push({
                             ...
                             action: function (e, dt, node, config, cb) {
                                 ...
                                 $('#' + id).prop('checked', function (_, oldProp) {
                                     if (oldProp) {
             window.LaravelDataTables['users-table'].columns(index + 1).search("").draw();
                 ...
                 arrOfFilterBtn.push({
                     ...
                     action: function(e, dt, node, config, cb) {
                         ...
                         $('.filter-modal input[type="text"]').on('input', function() {
                             ...
                             window.LaravelDataTables['users-table'].columns(filterIndex).search(searchValue).draw();
                         });
                     }
                 });
       ...
       $('.selected-item').text(window.LaravelDataTables['users-table'].rows({ selected: true }).count());
       $('.selected-badge').text(window.LaravelDataTables['users-table'].rows({ selected: true }).count());
     },lengthMenu: [
         [10, 25, 50,100,200],
         [10, 25, 50,100,200]
     ]",
